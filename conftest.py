...
def _mutating_gate(request: pytest.FixtureRequest) -> None:
    ...
    **마커만으로는 부족하기 때문에 있다.** 마커는 "고를 수 있게" 해주는 것이지
    ...
    ★ 2026-09-10 정정 - 여기에 *"이 fixture 는 의존이 없어서 `logged_in_page` 보다 먼저
      돌고, 그래서 차단될 때는 브라우저가 아예 안 뜬다"* 고 적혀 있었다. **근거도 결론도
      틀렸다.**

        근거   pytest 는 의존보다 **스코프를 먼저** 본다(session > module > function).
               `auth_state` 가 session 이라, 이 fixture 가 의존이 없든 autouse 든
               **항상 `auth_state` 뒤에 돈다.** "의존이 없다" 는 "먼저 돈다" 가 아니다.
        결론   skip 이 나면 남은 fixture 는 진짜로 안 만들어지지만, 그때 안 만들어지는
               것은 `logged_in_page` 뿐이다. **`auth_state` 는 이미 돈 뒤**라 브라우저가
               떴고, 세션이 죽어 있으면 2FA 창까지 뜬다.
        착시   09-09 관찰 당시엔 `auth.json` 이 신선해서 `auth_state` 가 아무것도 안 했다.
               게이트가 먼저 돈 게 아니라 `auth_state` 가 조용했던 것이다.

      **안전에는 영향이 없다** - 순서가 어떻든 게이트는 항상 발동하고 데이터를 바꾸는
      코드까지는 안 간다. 순서가 바꾸는 것은 "skip 하기 전에 로그인을 한 번 하느냐" 뿐이고,
      그게 낭비가 되는 명령은 `pytest -m mutating -v`(플래그 없이) 하나뿐이다. 다른 경로는
      읽기 전용 TC 가 섞여 있어 어차피 로그인이 필요하다 - `pytest -v` 도,
      `pytest test_vehicle_company_transfer.py`(TC-057·058 이 mutating 이 아니다)도 그렇다.

      **언제 고칠 값어치가 생기나** - `-m mutating` 만 따로 돌리는 일이 잦아지면 이 판정을
      `pytest_runtest_setup` 훅으로 옮긴다. 그 자리는 **모든 fixture 보다 먼저** 돌아서
      위에 적혀 있던 문장이 그제서야 사실이 된다. 지금은 그 한 명령을 위해 안전장치를
      건드리는 것이라 이득 대비 위험이 안 맞는다.
