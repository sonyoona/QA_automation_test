...
def _select_reseller(page, value):
...
def _find_list_col_index(page, header_text):
...
def test_TC051_vehicle_edit_partner_field_visible(logged_in_page):
    """
    TC-051 | 차량 수정 시 파트너 항목 출력 확인
...
def test_TC052_vehicle_edit_reseller_field_visible(logged_in_page):
    """
    TC-052 | 차량 수정 시 리셀러 항목 출력 확인
...
def test_TC053_vehicle_edit_reseller_change_and_revert_when_partner_lg_uplus(logged_in_page):
    """
    TC-053 | 업체 선택 변경에 따른 리셀러 값 초기화/재설정
...
def test_TC054_vehicle_edit_reseller_auto_selected_when_partner_connect(logged_in_page):
    """
    TC-054 | 차량 수정 시 파트너 기준 [리셀러 선택 제한] 확인 (파트너가 커넥트인 경우)
...
def test_TC055_vehicle_edit_reseller_required_when_partner_lg_uplus(logged_in_page):
    """
    TC-055 | 차량 수정 시 파트너 기준 [리셀러 선택 제한] 확인 (파트너가 LG U+인 경우)
...
def test_TC056_vehicle_edit_reseller_auto_selected_when_partner_smallticket(logged_in_page):
    """
    TC-056 | 차량 수정 시 파트너 기준 [리셀러 선택 제한] 확인 (파트너가 스몰티켓인 경우)
