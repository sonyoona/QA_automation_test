...
class NewServicePopup:
    ...
    def request_rows(self) -> Locator:
        ...
        팝업을 막 열면 목록이 비어서 이 행 요소가 DOM 에 없다 - 1건을 담아야 생긴다.

        ★ 접두어 이름은 맞았지만 **뒤에 붙는 값이 표 행과 다르다** (2026-09-08 실측).

            fs-table__row--10679            <- 서버가 준 그 건의 id
            fs-new__request-list-row--0     <- 0 부터 세는 화면상의 순번
            fs-new__request-list-row--1

        아직 서버에 보내기 전이라 id 가 있을 수 없다. 그래서 **신원이 아니라 위치**다 -
        중간 행을 지우면 뒤 행의 번호가 당겨진다. 개수를 세거나 "N 번째 담긴 행" 을
        가리키는 데까지만 쓰고, 조작 전후로 같은 행을 다시 찾는 데는 쓰지 않는다
        (CLAUDE.md "행 식별" 장의 4순위 - 인덱스는 같은 렌더 안에서만).
        """
