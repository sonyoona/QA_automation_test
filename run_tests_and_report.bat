...
REM
REM   run_tests_and_report.bat                    읽기 전용 60건만 (기본)
REM   run_tests_and_report.bat --allow-mutating   저장하는 9건까지 69건 전부
REM
REM 붙인 인자는 %* 로 pytest 에 그대로 넘어간다. 데이터를 바꾸는 9건
REM (@pytest.mark.mutating) 은 게이트가 기본적으로 skip 하므로, 플래그 없이
REM 돌리면 결과에 9 skipped 가 뜬다 - 고장이 아니다.
