...
# Allure 가 서로 다른 실행의 같은 TC 를 'Retries' 로 묶어 보여준다 - 이번에 skip 된
# TC 를 눌렀는데 지난 실행의 실패가 나와서 '안 돌린 TC 가 왜 빨간불이냐' 가 된다
# (2026-09-10 실측 - 결과 폴더에 5개 실행분 147건이 섞여 있었다).
# allure-report 는 원래 매번 지우고 있었는데 allure-results 만 빠져 있었다.
pytest -v --alluredir=allure-results --clean-alluredir "$@" || true
