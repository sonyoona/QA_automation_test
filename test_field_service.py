...
def test_status_tab_filters_rows(fs: FieldServicePage, tab: str) -> None:
    ...
    **한 방향만 보면 반쪽이다** - ①만 보면 필터가 아무것도 안 돌려줘도(0건) 통과하고,
    ...
    ② 의 전제 - 두 목록이 같은 기준으로 정렬돼 있다는 것. 목록 조회 API 가
    `sort=requestedDateTime,desc`(작업신청일 내림차순)로 나가는 것을 2026-09-08 에
    확인했다. 그러면 "[전체] 1페이지에 있는 X 상태 행" 은 반드시 "[X] 탭 1페이지" 안에
    있다 - 전체에서 그 행보다 위에 있는 행이 9개 이하이므로 X 중에서도 9번째 안이다.
