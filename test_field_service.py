...
# 배경: docs/notes/code-notes/현장서비스-테스트-노트.md · 한 줄씩 읽기: 현장서비스-코드-한줄씩.md
...
# 이 화면은 배포 대상이 아니라 자동화 연습용이라 260907 을 **확정 번호로 고정**한다 - 안 바꾼다.
# 배포가 없는데 "임시" 로 남겨두면 다음 사람이 바꿔야 하는 것으로 읽는다.

# 액션 버튼 라벨은 한 가지가 아니라 행 상태에 따라 갈린다 (2026-09-08 실측 50행).
#   진행 중(대기·배정완료) -> [수정]      종료(완료·취소·작업불가) -> [상세]
# 기획서로 확인한 규칙이 아니라 관찰이라 어긋나면 화면이 바뀐 것 - 실패가 맞다.
# 경위: docs/notes/code-notes/현장서비스-테스트-노트.md "라벨은 한 가지가 아니었다"
...
def _expect_rendered(locator: Locator, what: str) -> None:
    ...
    `is_visible()`·`inner_text()` 는 재시도가 없어 안 그려진 화면을 전부 "없음" 으로 읽는다.
    한글 이름을 담아 다시 던지는 이유(expect 기본 메시지는 셀렉터만 보여준다):
    docs/notes/code-notes/현장서비스-테스트-노트.md "그 첫 항목만 이름 없이 실패하고 있었다"
    """
    try:
...
def test_table_rows_have_action_button(fs: FieldServicePage) -> None:
    ...
    ★ 버튼을 누르지 않는다 - [수정] 은 수정 화면 입구라 노출·라벨까지만 본다.
    행 식별을 PK 로 하는 이유, testid 존재만 보면 안 되는 이유:
    docs/notes/code-notes/현장서비스-테스트-노트.md "TC-260907-003"
    """
...
def test_action_label_by_status(fs: FieldServicePage, tab: str) -> None:
    ...
    TC-260907-003 이 [전체] 1페이지만 보는 구멍을 메운다. "탭 필터가 걸렸는가" 는 일부러
    안 본다(그건 TC-260907-007 몫): docs/notes/code-notes/현장서비스-테스트-노트.md "대응표를 채우는 것과 검증되는 것은 다르다"
    """
...
def test_status_tab_filters_rows(fs: FieldServicePage, tab: str) -> None:
    ...
    ★ 아무것도 누르지 않는다 - 탭 클릭은 조회다.
    한 방향만 보면 반쪽인 이유와 ②의 정렬 전제(`sort=requestedDateTime,desc`):
    docs/notes/code-notes/현장서비스-테스트-노트.md "필터는 두 방향을 같이 걸어야 한다"
    """
...
def test_new_service_popup_fields_visible(fs: FieldServicePage) -> None:
    ...
    `assert` 를 `close()` 보다 먼저 두는 이유(실패 스크린샷에 팝업이 남아야 한다):
    docs/notes/code-notes/현장서비스-테스트-노트.md "팝업 TC 는 assert 를 close() 보다 먼저 둔다"
    """
