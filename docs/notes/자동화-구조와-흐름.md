...
★ **2026-09-10 에 `main.py` 와 `AGENTS.md` 를 지웠습니다.** `main.py` 는 PyCharm 이 프로젝트를
만들 때 넣어준 샘플이라 자동화와 관계가 없었고, `AGENTS.md` 는 `CLAUDE.md` 의 9월 3일자
옛 사본이었습니다 — 데이터를 바꾸는 TC 도 마커·게이트도 적혀 있지 않아, 그것만 읽으면
**`pytest -v` 가 전부를 돈다고 믿게** 됩니다. `AGENTS.md` 는 다른 AI 도구가 자동으로
읽는 파일명이라 사람이 안 열어도 기계가 엽니다. **규칙은 `CLAUDE.md` 하나가 정본입니다.**
...
**스크립트에 붙인 인자는 pytest 로 그대로 넘어갑니다** (`.bat` 은 `%*`, `.sh` 는 `"$@"`).
그래서 게이트를 켜려면 뒤에 붙이기만 하면 됩니다 — 스크립트를 고칠 필요가 없습니다.

```bash
run_tests_and_report.bat                    # 읽기 전용 60건 (저장하는 9건은 skip)
run_tests_and_report.bat --allow-mutating   # 69건 전부 - dev 차량이 실제로 바뀐다
```

★ **스크립트와 아래 PowerShell 순서는 첫 줄이 다릅니다.** 스크립트는 `pytest -v` 라
  9건이 `skipped` 로 **리포트에 남고**, 아래 순서는 `-m "not mutating"` 이라 아예
  **안 남습니다.** 실제로 도는 것은 둘 다 60건으로 같고 - 차이는 리포트 모양뿐입니다.
  플래그 오타(`--allow-mutatng`)는 pytest 가 `unrecognized arguments` 로 즉시 끊으므로,
  **켰다고 생각했는데 안 켜진 채로 도는 일은 없습니다.**
...
```powershell
# 평소에는 이걸 쓴다. 저장하는 9건은 게이트가 skip 하므로 -m 은 리포트 정리용이다
...
전부 돌리려면 첫 줄만 `pytest --allow-mutating -v --alluredir=allure-results` 로 바꿉니다 —
플래그 없이 `pytest -v` 만 쓰면 게이트가 9건을 skip 해서 "전부" 가 되지 않습니다.
**dev 차량 5대(이관 4대 + TC-053 1대)의 값이 실제로 바뀌었다 돌아온다는 뜻**이라는 것을 알고 돌립니다.
...
- **실행 주체가 종류마다 다릅니다** (2026-09-09 개정). 계정이 단일 세션이라 아무나 임의로
  돌리면 상대가 열어 둔 화면이 끊기기 때문입니다.

  | 무엇을 | 누가 |
  |---|---|
  | 실측 스크립트 (`test_probe_*`, codegen) | Claude — **"브라우저로 로그인해 둔 것 없나" 를 묻고 확인받은 뒤** 돌린다 |
  | 브라우저를 안 여는 것 (`--collect-only` · git · 마커 확인) | Claude — 세션을 안 쓰므로 그냥 돌린다 |
  | 전체 실행 · 리포트 생성 | 사용자 — 20분 이상 걸려 그동안 화면을 못 쓴다 |
  | mutating 실행 (`--allow-mutating`) | 사용자 — 데이터를 바꾸므로 사람이 의도를 갖고 켠다 |
...
(`900용1001~1004`)만 쓰고, `company_guard` fixture 의 **teardown** 이 원래 업체로 되돌려
반복 실행이 가능합니다 — 경위는 `code-notes/차량업체변경-테스트-노트.md` 참고.

그래서 **"이 저장소의 테스트는 안전하다" 를 전제로 아무 파일이나 돌리면 안 됩니다.**
지금 어떤 장치로 막고 있는지를 아래에 적어 둡니다.

**2026-09-09 에 그 8건에 `@pytest.mark.mutating` 을 붙였습니다.** 이제 골라 돌릴 수 있습니다.

```bash
pytest -v                      # 60건 실행 + 저장하는 9건은 게이트가 skip (기본값)
pytest -m "not mutating" -v    # 위와 같되 skip 조차 리포트에 안 남긴다
pytest -m mutating --allow-mutating -v   # 저장하는 9건만 - 플래그가 있어야 돈다
# 건수는 2026-09-11 기준 - 09-10 에 TC-053 이 mutating 에 들어와 61/8 에서 60/9 가 됐다
```
...
0.87s**. 경위는 `code-notes/차량업체변경-테스트-노트.md` 참고.

★ 여기에 `(브라우저도 안 뜬다)` 라고 덧붙여 뒀는데 **2026-09-10 에 정정했습니다.**
그날 안 뜬 건 `auth.json` 이 신선해서 `auth_state` 가 조용했기 때문이지, 게이트가 먼저
돌아서가 아닙니다 — pytest 는 의존보다 **스코프를 먼저** 봐서 session 스코프인
`auth_state` 가 function 스코프인 게이트보다 **항상 먼저** 돕니다.
**안전에는 영향이 없습니다**(게이트는 순서와 무관하게 항상 막습니다). 자세한 것은
`code-notes/차량업체변경-테스트-노트.md` 의 정정 항목 참고.
