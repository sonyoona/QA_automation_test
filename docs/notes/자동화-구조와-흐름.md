...
pytest -m mutating -v          # 저장하는 8건만 - .env 를 켜야 실제로 돈다
```

**같은 날 옵트인 게이트도 넣었습니다.** `conftest.py` 의 `_mutating_gate`(autouse fixture)가
`mutating` 마커를 보고, `.env` 에 `ALLOW_MUTATING_TESTS=true` 가 없으면 setup 단계에서
스스로 skip 합니다. 마커는 "고를 수 있게" 하는 것이고 게이트는 **"기본값을 안 함으로 만드는"**
...
- `.env` 의 `ALLOW_MUTATING_TESTS` **옵트인 게이트는 없습니다.** 마커는 "고를 수 있게" 했을
  뿐이고, **모르고 돌리는 것을 막지는 못합니다** — `pytest -v` 는 여전히 8건을 같이 돕니다.
- 원복(원래 업체로 되돌리기)이 **fixture teardown 이 아니라 테스트 본문 마지막 줄**에 있어서,
  중간에서 실패하면 dev 차량이 바뀐 채로 남습니다. 다음 실행은 "지금 보이는 값" 을 기준으로
  다시 도니까 테스트가 깨지지는 않지만, **dev 데이터는 바뀐 채로 남습니다.**

`CLAUDE.md` 에는 "지금 있는 테스트는 전부 읽기 전용이라 아직 위험이 없다" 고 적혀 있었는데,
2026-09-09 에 위 사실을 확인하고 고쳤습니다 — `데이터를 바꾸는 동작` 장과
