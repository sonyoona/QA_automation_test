...
- `@pytest.mark.mutating` 마커도 `ALLOW_MUTATING_TESTS` 옵트인 게이트도 **아직 없습니다**
  (`CLAUDE.md` 의 "변경성 테스트 준비" 장에 설계만 있고 구현이 안 됐습니다).
  즉 `pytest -v` 를 그냥 돌리면 저장하는 TC 도 같이 돕니다.
- 원복(원래 업체로 되돌리기)이 **fixture teardown 이 아니라 테스트 본문 마지막 줄**에 있어서,
  중간에서 실패하면 차량이 바뀐 채로 남습니다.
