...
> **`conftest.py`의 `_login_and_save`를 그대로 재사용합니다**
>
> 원래는 이 테스트 안에 로그인 코드를 직접 다 썼는데, `conftest.py`에 세션 전체용 `auth_state` fixture를 만들면서 로그인 로직이 완전히 겹쳤습니다. 그래서 실제 동작은 `_login_and_save` 함수 하나로 합치고, 이 테스트는 그 함수를 호출만 합니다 — 코드가 두 군데 있으면 나중에 셀렉터 하나 고칠 때 한쪽을 까먹기 쉬워서요.
>
> 다만 **동작 방식은 다르게 유지**했습니다: `auth_state`는 `auth.json`이 신선하면 로그인을 건너뛰지만, 이 테스트(TC-114)는 "로그인 자체가 되는지" 검증하는 게 목적이라 **건너뛰지 않고 실행할 때마다 항상 실제 로그인을 수행**합니다. 그래서 `_login_and_save`를 직접 호출하지, `auth_state` fixture를 통해서 부르지 않습니다.
...
- **`browser_type`** — pytest-playwright가 주는 기본 fixture. "chromium/firefox/webkit 중 어떤 엔진을 쓸지"를 나타내고, `.launch()`로 새 브라우저를 하나 더 켤 수 있습니다.
- **`_login_and_save(browser_type)`** — `conftest.py`에 정의된 함수. 내부에서 로그인 전용 headed 브라우저를 따로 켜고(다른 테스트가 쓰는 기본 브라우저는 headless라 화면이 안 보이므로), 아이디·비밀번호 자동 입력 → 사람이 인증번호 직접 입력 → `auth.json` 저장까지 전부 처리합니다. 자세한 동작은 `conftest.py` 자체를 참고하세요.
- 이 테스트 자체엔 `--headed`가 필요 없습니다 — `_login_and_save`가 로그인 전용 브라우저를 코드로 직접 headed로 켜기 때문에, 터미널 옵션과 무관하게 항상 화면이 뜹니다.
...
3. **실행**
   ...
   ```
   `--headed`는 이제 안 주셔도 됩니다 — TC-114는 `_login_and_save`가 로그인 전용 브라우저를 코드로 직접 headed로 켭니다. `-s`는 print 출력이 안 가려지게 하는 옵션으로, 사람이 개입할 타이밍(인증번호 입력 안내 메시지)을 놓치지 않는 데 도움이 됩니다.
4. **TC-114 실행 중엔 자리를 지키기** — 새로 뜬 로그인 전용 브라우저 창에서 인증번호 입력이 필요해지면, 터미널에 안내 메시지가 뜹니다. 그 창에서 **직접 인증번호를 입력하고 [확인]을 눌러주세요.** 그러면 자동으로 이어서 진행되고 `auth.json`이 갱신됩니다.
