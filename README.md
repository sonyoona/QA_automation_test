...
├── test_field_service.py             # 현장 서비스 화면 노출 검증 (testid 기반)
├── pages/                            # Page Object - 셀렉터(testid)가 적히는 곳
│   └── field_service_page.py         # 현장 서비스 목록 화면 + 신규 서비스 신청 팝업
├── docs/                             # 테스트 관련 문서
...
>
> **전체가 어떻게 맞물려 도는지**(실행 흐름·fixture 사슬·대기·리포트 파이프라인)는 [`docs/notes/자동화-구조와-흐름.md`](./docs/notes/자동화-구조와-흐름.md)에 그림과 함께 있습니다 — 코드를 처음 여는 사람이 먼저 볼 곳입니다.
>
> **테스트 내용이 아니라 실행 자체가 안 될 때**(배치 파일이 깨짐 · 전 테스트 ERROR · 2FA 창이 안 뜸 · 줄바꿈/인코딩)는 [`docs/notes/실행-트러블슈팅-노트.md`](./docs/notes/실행-트러블슈팅-노트.md)에 증상별로 정리해 뒀습니다.
...
차량관리 > 현장 서비스 화면의 주요 영역·조회 필터·결과 테이블·신규 서비스 신청 팝업이 정상 노출되는지 검증합니다. 전부 읽기 전용 TC 입니다 — [신청 완료]는 누르면 dev에 실제 신청 건이 생성되므로 노출 여부만 확인합니다.

- 화면 진입 시 상태 탭·신규 신청 버튼·결과 테이블·엑셀 다운로드 버튼 노출 확인
- 조회 필터 13개 항목 노출 확인 (빠진 항목은 이름으로 전부 보고)
- 결과 행마다 자기 PK를 가진 액션 버튼이 있고, 행 상태에 맞는 라벨인지 확인
  (진행 중이면 [수정], 끝난 건이면 [상세] — 상태 탭 5개를 돌며 확인)
- 상태 탭 필터가 그 상태 건만, 빠짐없이 조회하는지 확인 (섞임·누락 두 방향)
...
| TC-260907-001 | 현장 서비스 화면 진입 시 주요 영역 노출 확인 | 정상 케이스 |
| TC-260907-002 | 현장 서비스 조회 필터 13개 항목 노출 확인 | 정상 케이스 |
| TC-260907-003 | 결과 행마다 액션 버튼([수정]/[상세])이 하나씩 노출되는지 확인 | 정상 케이스 |
| TC-260907-004 | 신규 서비스 신청 팝업 입력 15개 항목 노출 확인 | 정상 케이스 |
| TC-260907-005 | 신규 서비스 신청 팝업 하단 버튼 노출 및 닫기 복귀 확인 | 정상 케이스 |
| TC-260907-006 | 상태 탭 5개를 돌며 행 상태에 맞는 액션 버튼 라벨이 노출되는지 확인 | 정상 케이스 |
| TC-260907-007 | 상태 탭이 그 상태 건만·빠짐없이 조회하는지 확인 (오검출/누락 두 방향) | 정상 케이스 |
